    ...
    public function getProduct(){
        $Product = Products::findorfail(request('product_id'));
         return $data=     '<tr>
                                ...
                                <th>'.((($Product->Selling_Price*$Product->CGST)/100)*request('qty')). '<input type="hidden" value="' . request('qty') . '" name="CGST[]"></th>
                                <th>'.((($Product->Selling_Price*$Product->SGST)/100)*request('qty')). '<input type="hidden" value="' . request('qty') . '" name="SGST[]"></th>
                                <th>'.((($Product->Selling_Price*@$Product->CESS)/100)*request('qty')). '<input type="hidden" value="' . request('qty') . '" name="CESS[]"></th>
